...
exports.getReviews = async (req, res) => {
  ...
  try {
    const reviews = await Review.find({ companyId }).lean();
    res.json(reviews);
  } catch (error) {
...
exports.addReview = async (req, res) => {
  const { companyId, name, subject, reviewText, rating } = req.body;
  const validationError = validateObjectId(companyId, res);
  if (validationError) return;
  ...
  try {
    ...
    const review = await Review.create({ companyId, reviewerName: name, subject, reviewText, rating });
...
exports.getSearchReviews = async (req, res) => {
  ...
  try {
    ...
    if (!isNaN(query) && query.trim() !== "") {
      searchConditions.push({ rating: Number(query) });
    }
