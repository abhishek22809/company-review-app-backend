...
exports.getCompaniesByCity = async (req, res) => {
  try {
    ...
    if (city || name) {
      query.$or = [];
      if (city) query.$or.push({ location: new RegExp(city, "i") });
...
exports.getAllCompanies = async (req, res) => {
  try {
    const companies = await Company.find().populate("reviews", "rating") .sort({ _id: -1 }).lean();
    res.status(200).json(companies);
  } catch (error) {
...
exports.getCompanyById = async (req, res) => {
  ...
  try {
    const company = await Company.findById(id).populate("reviews", "rating") .sort({ _id: -1 }).lean();
...
exports.addCompany = async (req, res) => {
  try {
    const { name, location, foundedOn } = req.body;
